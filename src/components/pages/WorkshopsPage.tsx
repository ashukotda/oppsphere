...
export default function WorkshopsPage() {
  ...
  useEffect(() => {
    filterOpportunities();
  }, [opportunities, searchTerm, locationFilter, compensationFilter, skillLevelFilter]);

  const loadOpportunities = async () => {
    setIsLoading(true);
    try {
      const result = await BaseCrudService.getAll<Opportunities>('opportunities');
      const workshops = result.items.filter((opp) => opp.category?.toLowerCase() === 'workshop');
      setOpportunities(workshops);
    } catch (error) {
        ...
        <div className="max-w-[100rem] mx-auto px-8">
          ...
          >
            ...
            <p className="font-paragraph text-xl md:text-2xl text-dark-grey/80">
              Enhance your skills with hands-on workshops and training programs.
            </p>
          </motion.div>
        </div>
      </section>
            ...
            {isLoading ? null : filteredOpportunities.length > 0 ? (
                    ...
                    {opportunity.companyLogo && (
                      <div className="w-full h-48 bg-background flex items-center justify-center p-8">
                        <Image
                          ...
                          alt={opportunity.companyName || 'Workshop logo'}
                          width={200}
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        {opportunity.compensation && (
                          <span className="inline-block px-3 py-1 bg-secondary/10 text-secondary text-xs font-semibold rounded-full font-paragraph">
                            {opportunity.compensation === 'paid' ? 'Paid' : 'Free'}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-2 mb-6">
                        ...
                        {opportunity.startDate && (
                          <div className="flex items-center gap-2 text-sm text-dark-grey/70 font-paragraph">
                            ...
                            <span>Starts: {format(new Date(opportunity.startDate), 'MMM dd, yyyy')}</span>
                          </div>
                        )}
                      </div>
              ...
              <div className="text-center py-20">
                <p className="font-paragraph text-lg text-dark-grey/70">
                  No workshops found matching your filters. Try adjusting your search criteria.
                </p>
              </div>
            )}
          </div>
